...
class OrderRepository : IOrderRepository {
    ...
    override suspend fun getAll(userId: String, search: String): List<Order> = suspendTransaction {
        if (search.isBlank()) {
            OrderDAO
                .find { OrderTable.userId eq UUID.fromString(userId) }
            ...
            OrderDAO
                .find {
                    (OrderTable.userId eq UUID.fromString(userId)) and
    ...
    override suspend fun update(userId: String, orderId: String, newOrder: Order): Boolean = suspendTransaction {
        val dao = OrderDAO
            .find {
                (OrderTable.id eq UUID.fromString(orderId)) and
                (OrderTable.userId eq UUID.fromString(userId))
            }
            .limit(1)
            .firstOrNull()
